...
function paginate($reload, $page, $tpages, $adjacents) {
	...
	if($page==1) {
		$out.= "<li class='disabled'><button class='btn btn-sm btn-info'><a>$prevlabel</a></button></li>";
	} else if($page==2) {
	...
	for($i=$pmin; $i<=$pmax; $i++) {
		if($i==$page) {
			$out.= "<li class='active'><a class='btn btn-sm'>$i</a></li>";
		}else if($i==1) {
	...
	if($page<$tpages) {
		...
		$out.= "<li class='disabled'><button class='btn btn-sm btn-info'><a>$nextlabel</a></button></li>";
	}
