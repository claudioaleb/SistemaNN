            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h5>ABASTECIMIENTO POR PROVEEDOR</h5>
                    </div>
...
<form NAME="selecciona" action="abastecimiento-proveedor.php" method="post">
...
<span class="input-group-btn">
       ...
       <br>
           ...
           <button type="submit" name="Limpiar" class="btn btn-default btn-sm">
               <i class="icon-repeat"></i> Reiniciar
           </button>

           <button type="submit" name="Consultar" value="Consultar" class="btn btn-success  btn-sm">
               <i class="icon-eye"></i> Consultar Productos
           </button>
...
if (isset($_POST["Consultar"])) {
    ...
    $rela_proveedor = $_POST["id_proveedor"];
  
$cn = mysqli_query($conexion,"select * from stock join productos on rela_producto=id_producto join proveedores on rela_proveedor=id_proveedor WHERE rela_proveedor='$rela_proveedor'");

  echo "<div class=table-responsive-sm>";
  echo "<table class=table table-sm table-striped table-bordered table-hover table-condensed>";
  echo "<thead>";
  echo "<tr> <th>Proveedor</th><th>Producto</th><th>Stock</th><th>Nº Comprobante</th><th>Precio/U</th><th>Cantidad</th><th>Costo</th><th>Precio Venta</th> </tr>";
while ($reg= mysqli_fetch_array($cn))
 {
 echo '<tr">';
 
 echo '<td style=width:22%;>';
 echo $reg['razon_social'].', '.$reg['nombre_proveedor'] ;
 echo '</td>';
 ...
 <form method="post" action="alta-abastecimiento-proveedor.php" id="register-form" class="form-sm">
              
              <td>
                 <input name="id_compra" id="id_compra" class="form-control input-sm" required="true" placeholder="Nº Factura" style='width:88%;' autocomplete=off>
              </td>

              <td>
                 <input type="number" name="precio_unitario" id="precio_unitario" class="form-control input-sm" required="true" placeholder="$" style='width:80%;' autocomplete=off>
              </td>

              <td>
                 <input type="number" name="cantidad_ingreso" id="cantidad_ingreso" class="form-control input-sm" required="true" style='width:80%;' autocomplete=off>
              </td>

              <td>
                 <input type="number" name="total" id="total" class="form-control input-sm" required="true" placeholder="$" style='width:80%;' autocomplete=off>
              </td>

              <td>
                 <input type="number" class='form-control input-sm' name="precio_venta" id="precio_venta" required="true" placeholder="$" style='width:80%;' autocomplete=off>
              </td>

                 <input type="hidden" name="rela_producto" id="id_producto" class="form-control input-sm" value="<?php echo $reg['id_producto']; ?>">
                 <input type="hidden" name="rela_proveedor" id="id_proveedor" class="form-control input-sm" value="<?php echo $reg['id_proveedor']; ?>">
              
              <td>
                 <button class="btn btn-default" type="submit"><i class='icon-add-solid' style='color: blue'></i></button>
              </td>

              <td>
               <!-- Cuadro de texto confirmacion de alta de stock -->
               <div id="resultado"></div>
               <!-- FIN Cuadro de texto confirmacion de alta de stock -->
              </td>
