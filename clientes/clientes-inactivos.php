...
<?php
    ...
    if (@mysqli_connect_errno()) {
        ...
        $reload = 'mantenimiento-cliente.php';
        //consulta principal para recuperar los datos
        $query = mysqli_query($con,"SELECT * FROM clientes JOIN persona ON clientes.rela_persona = id_persona  WHERE estado_cliente = 'Inactivo'  ORDER BY apellido_persona, nombre_persona  ASC LIMITASC LIMIT $offset,$per_page");
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">MANTENIMIENTO DE CLIENTES INACTIVOS</h1>
                    </div>
...
-->
    <div class="panel panel-default" align="center">
        <div class="panel-body mb-4" align="right">
    <button type="button" class="btn btn-success" onclick="location.href='mantenimiento-cliente.php'"><span class="icon-list1"></span> Clientes Activos</button>
</div>
        <div class="panel-body mb-4" align="left">

            <div class="input-group">

                <button type="button" class="btn btn-success" onclick="location.href='formulario-alta-cliente.php'"><span class="icon-user-add"></span> Añadir Cliente</button>
...
JOIN persona ON clientes.rela_persona = id_persona 
JOIN localidades ON persona.rela_localidad = id_localidad
JOIN provincias ON persona.rela_provincia = id_provincia
JOIN domicilioxpersona ON persona.rela_domxperson = id_domicilioxpersona
JOIN domicilio ON domicilioxpersona.rela_domicilio = id_domicilio
JOIN contactoxpersona ON persona.rela_contacto = id_contacto_persona
JOIN tipo_contacto ON contactoxpersona.rela_tipo_contacto = id_tipo_contacto
 WHERE estado_cliente ='Inactivo'
ORDER BY apellido_persona, nombre_persona ASC");
            ...
            ?>
                <tr>
                    ...
                    <td><?php echo $row['apellido_persona'].', '.$row['nombre_persona'];?></td>
                    <td><?php echo $row['dni_persona'];?></td>
                    <td><?php echo $row['fecha_nacimiento'];?></td>
                    <td><?php echo $row['barrio'].', '.$row['calle'].' ,'.$row['altura'] ;?></td>
                    <td><?php echo $row['nombre_provincia'];?></td>
                    <td><?php echo $row['descripcion_tipo_contacto'].', '.$row['dato_contacto'];?></td>
                    <td><?php if ($row['estado_cliente'] == 'Activo'){ 
                        echo '<span class="label label-success">Activo</span>'; } 
                        else if ($row['estado_cliente'] == 'Inactivo' ) { 
                        echo '<span class="label label-danger">Inactivo</span>';} ?></td>
                    <td><?php echo $row['fecha_alta'];?></td>

                   <td align="center"> 
                        <button class="btn btn-success btn-sm agregar"
                            id_cliente="<?php echo $row['id_cliente'];?>"
                            type="button" data-toggle="modal" data-target="#agregarCliente"><i class='icon-add-outline'></i></button>
                    </td>
                    <td align="center"> 
                        <button class="btn btn-danger btn-sm borrar"
                            id_cliente="<?php echo $row['id_cliente'];?>"
                            type="button" data-toggle="modal" data-target="#eliminarCliente"><i class='icon-trash'></i></button>
                    </td>

                    <?php include_once 'modal-alta-cliente.php';?>
                    <?php include_once '../acciones/eliminar-cliente.php';?>
